...
@then("the {outcome} should be {relationship} {control}")
def step_impl(context, outcome, relationship, control):
    ...
    assert (
        outcome in dag.nodes()
    ), f"Outcome {outcome} not in graph nodes. Must be one of {dag.nodes()}"
    assert (
        outcome in data
    ), f"Outcome variable {outcome} not in data. Must be one of {data.columns}."
    ...
    if hasattr(context, "estimates_file"):
        with open(context.estimates_file, "a") as f:
            print(
                f"{context.feature_name},{context.scenario.name},{context.treatment_var},{outcome},{context.control_val},{context.treatment_val},{estimate},{ci_low},{ci_high},{relationship},{'pass' if test_bool(estimate, relationship, ci_low, ci_high) else 'fail'}",
                file=f,
            )
...
def step_impl(context, treatment_var, treatment_val):
    if context.treatment_var != treatment_var:
        raise ValueError(
            f"Specified treatment variable {treatment_var} is not the same as the one in the Given ({context.treatment_var})"
        )
    context.treatment_val = (
        context.types[treatment_var](treatment_val)
        if treatment_var in context.types
        else float(treatment_val)
        if treatment_val.isnumeric()
        else treatment_val
    )
