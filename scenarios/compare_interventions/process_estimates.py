...
def errorMetric(actual, estimated):
    ...
    assert len(actual) == len(estimated), "length of lists must be equal"
    return [abs(e-a) for a, e in zip(actual, estimated)]
...
for sample in sorted([int(x) for x in os.listdir(base) if x != "rct"]):
    ...
    errors = [sum(errorMetric(rct_estimates, d['estimate'])) for d in data]
    estimationErrors.append(np.mean(errors))

fig, ax1 = plt.subplots()
color = 'tab:red'
ax1.set_xlabel("Number of data points")
ax1.set_ylabel("Estimation error", color=color)
ax1.plot(range(50, 1050, 50), estimationErrors, color=color)
