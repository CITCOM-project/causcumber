...
parser.add_argument(
    "--n_days", type=int, nargs=1, help="Number of days to run the sim.", required=True
)
parser.add_argument(
    "--quar_period", type=int, nargs=1, help="The quarantine period.", required=True
)
parser.add_argument(
    "--pop_size", type=int, nargs=1, help="The size of the population", required=True
)
parser.add_argument(
    "--pop_infected",
    type=int,
...
parser.add_argument(
    "--trace_probs", type=float, nargs=1, help="The tracing probability.", required=True
)
parser.add_argument(
...
for i in range(args.n_runs[0]):
    ...
    sim.run()
    results = {k: [] for k in args.desired_outputs}
    results["quar_period"] = []
    df = sim.to_df()
    quar_period = 14
    ...
    df = df[args.desired_outputs]

    for k in args.desired_outputs:
...
pd.concat(all_runs).reset_index().to_csv(args.results_file[0])
