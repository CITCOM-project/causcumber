...
class SelectScenario(Screen):
    ...
    def list_scenarios(self):
        ...
        for item in os.listdir(path='.'):
            ...
            if (path.isdir('dags') and path.isdir('features')) == True:
                isScenario = True
...
class NewScenario(Screen): 
    ...
    def create_scenario(self, instance):
        ...
        environment_content = f.read()
        f.close()
        f = open("dagStepsTemplate.txt", "r")
        dag_steps_content = f.read()
        f.close()
        f = open("abstractPyTemplate.txt", "r")
        ...
        if not os.path.exists(path):
            ...
            for folder_name in basic_folders:
                ...
                if folder_name == 'features':
                    os.chdir('features')
                    environment_content = environment_content.replace('<tested_model_name>', new_directory)
                    f = open("environment.py", "w")
                    f.write(environment_content)
                    f.close()
                    os.mkdir(os.path.join(os.getcwd(), "steps"))
                    os.chdir('steps')
                    f = open("dag_steps.py", "w")
                    f.write(dag_steps_content)
                    f.close()
